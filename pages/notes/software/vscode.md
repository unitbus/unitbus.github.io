...
`.code-workspace`の位置ではなく、登録してるそれぞれのフォルダ直下の`.env`が適用されます。

シングルワークスペースの場合は、設定ファイルの場所が選べませんでしたが、
マルチワークスペースになって、自由に設定ファイルを置けるようになったので、共有する時に楽になりました。

ただ、注意点として、マルチワークスペースに追加したフォルダは`folders`オプションに記載されるのですが、
相対パスは使えのですが、環境変数が使えない点です。

なのでまったく関係ない場所に`.code-workspace`ファイルを置くと、絶対パスで書くしか無く、
ユーザー名フォルダとか挟むと詰みます。これは今後改善して頂きたい点です。
